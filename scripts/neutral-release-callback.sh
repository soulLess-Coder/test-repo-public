...
#
# Drop this at `scripts/neutral-release-callback.sh` in the public mirror
# repo (chmod +x). The workflow invokes it after Copybara/build/publish
# (success path) or in the `if: failure()` step (failure path).
#
...
# HMAC-SHA256 the body with the shared callback secret. openssl is available
